...
| `product_showcase` | Product Showcase | No | Clean product highlight, polished. |
...
| `wild_card` | Wild Card | No | Experimental, model picks the vibe. |
| `ugc_virtual_try_on` | UGC Virtual Try On | Yes | Person trying on clothing/accessories — UGC vibe. |
| `virtual_try_on` | Pro Virtual Try On | No | Same but more polished, model-driven. |

The "Hook/setting" column shows whether `--hook_id` and `--setting_id` are valid for that mode. Modes marked `No` ignore or reject setup items.
